...
CREATE TABLE durak_player (
    ...
    CONSTRAINT ck_dp_type CHECK (player_type IN ('HUMAN', 'SYSTEM')),
    CONSTRAINT ck_dp_identity CHECK (
        (player_type = 'HUMAN' AND db_username IS NOT NULL)
        OR (player_type = 'SYSTEM' AND db_username IS NULL)
    ),
...
CREATE TABLE durak_event (
    ...
    CONSTRAINT ck_de_source CHECK (
        event_source IN ('MANUAL', 'SYSTEM', 'TIMEOUT', 'ADMIN')
    ),
    CONSTRAINT ck_de_type CHECK (
        event_type IN (
            ...
            'GAME_CANCELLED', 'TIMEOUT', 'IDLE_TIMEOUT', 'HEARTBEAT'
        )
    ),
