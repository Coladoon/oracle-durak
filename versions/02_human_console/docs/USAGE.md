...
## Упрощённая игровая консоль

Текущее состояние можно получить одной строкой:
...
Команды принимаются на русском. `GAME_ID` можно не передавать, если у пользователя
ровно одна активная партия:
...
Если отправить только код карты, например `8H`, консоль определит атаку, защиту
или подбрасывание по текущей фазе. Полный понятный журнал доступен в `V_LOG_TEXT`.
