...
CREATE OR REPLACE PACKAGE durak_api AUTHID DEFINER AS
    ...
    PROCEDURE start_tournament_match (
...
CREATE OR REPLACE PACKAGE BODY durak_api AS
    ...
    PROCEDURE continue_bots (
        p_game_id     IN NUMBER,
        p_max_actions IN NUMBER DEFAULT 500
    ) IS
    BEGIN
        durak_bot.play_until_human(p_game_id, p_max_actions);
    END continue_bots;
    ...
    BEGIN
        ...
        durak_engine.create_game(
            ...
            p_allow_throw_after_take => p_allow_throw_after_take,
            p_limit_by_defender_hand => p_limit_by_defender_hand,
    ...
    BEGIN
        ...
        durak_engine.add_bot(
            p_actor_player_id => v_player_id,
            p_game_id         => p_game_id,
            p_bot_level       => p_bot_level,
            p_bot_player_id   => p_bot_player_id,
            p_seat_no         => p_seat_no
        );

        COMMIT;
        success(
            'бот уровня ' || UPPER(TRIM(p_bot_level))
            || ' добавлен на место ' || p_seat_no
            || ', PLAYER_ID=' || p_bot_player_id || '.',
            p_message
        );
    EXCEPTION
        WHEN OTHERS THEN
            ...
            v_error := SQLERRM;
            ROLLBACK TO durak_api_call;
            record_failure(p_game_id, v_player_id, 'ADD_BOT', v_code, v_error, p_message);
            p_bot_player_id := NULL;
            p_seat_no := NULL;
            IF NOT is_business_error(v_code) THEN
                RAISE;
            END IF;
    ...
    BEGIN
        ...
        IF v_count = 0 THEN
            RAISE_APPLICATION_ERROR(-20806, 'Игрок не участвует в этой партии.');
        END IF;

        continue_bots(p_game_id, p_max_actions);
        COMMIT;
        success('автоматические действия ботов обработаны.', p_message);
    EXCEPTION
        WHEN OTHERS THEN
            v_code := SQLCODE;
            v_error := SQLERRM;
            ROLLBACK TO durak_api_call;
            record_failure(p_game_id, v_player_id, 'RUN_BOTS', v_code, v_error, p_message);
            IF NOT is_business_error(v_code) THEN
                RAISE;
            END IF;
    END run_bots;
    ...
    BEGIN
        ...
        durak_tournament_pkg.create_tournament(
            p_creator_player_id      => v_player_id,
            p_tournament_name        => p_tournament_name,
            p_tournament_format      => p_tournament_format,
            p_deck_size              => p_deck_size,
            p_game_variant           => p_game_variant,
            p_first_move_mode        => p_first_move_mode,
            p_max_pairs              => p_max_pairs,
        ...
        COMMIT;
        success(
            'создан турнир TOURNAMENT_ID=' || p_tournament_id || '.',
            p_message
        );
    EXCEPTION
        WHEN OTHERS THEN
    ...
    BEGIN
        SAVEPOINT durak_api_call;
        v_player_id := required_player_id;
        durak_tournament_pkg.join_tournament(v_player_id, p_tournament_id);
        COMMIT;
        success('игрок зарегистрирован в турнире.', p_message);
    EXCEPTION
        WHEN OTHERS THEN
            v_code := SQLCODE;
            v_error := SQLERRM;
    ...
    BEGIN
        ...
        COMMIT;
        success(
            'турнирный бот PLAYER_ID=' || p_bot_player_id || ' добавлен.',
            p_message
        );
    EXCEPTION
        WHEN OTHERS THEN
    ...
    BEGIN
        SAVEPOINT durak_api_call;
        v_player_id := required_player_id;
        durak_tournament_pkg.start_tournament(v_player_id, p_tournament_id);
        COMMIT;
        success('турнир запущен, сетка матчей сформирована.', p_message);
    EXCEPTION
        WHEN OTHERS THEN
            v_code := SQLCODE;
            v_error := SQLERRM;
    ...
    BEGIN
        ...
        durak_tournament_pkg.start_match(
            p_actor_player_id     => v_player_id,
            p_tournament_match_id => p_tournament_match_id,
            p_game_id             => p_game_id
        );

        -- Матч двух ботов должен иметь возможность завершиться без участия
        -- Oracle-пользователя, который сам может не входить в эту пару.
        continue_bots(p_game_id, 10000);
        COMMIT;
        success('турнирный матч запущен, GAME_ID=' || p_game_id || '.', p_message);
    EXCEPTION
        WHEN OTHERS THEN
            v_code := SQLCODE;
            v_error := SQLERRM;
    ...
    BEGIN
        SAVEPOINT durak_api_call;
        v_player_id := required_player_id;
        durak_tournament_pkg.cancel_tournament(v_player_id, p_tournament_id);
        COMMIT;
        success('турнир отменён.', p_message);
    EXCEPTION
        WHEN OTHERS THEN
            v_code := SQLCODE;
            v_error := SQLERRM;
