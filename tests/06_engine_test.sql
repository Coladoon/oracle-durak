...
BEGIN
    ...
    INSERT INTO durak_player (
        ...
        v_player1, 'Engine Test 1', 'SYSTEM'
    );

    INSERT INTO durak_player (
        player_id, display_name, player_type
    ) VALUES (
        v_player2, 'Engine Test 2', 'SYSTEM'
    );
    ...
    durak_engine.process_timeout(v_game1);
    ...
    durak_test.assert_text(
        'Таймер подбрасывающего делает пас и завершает раунд',
        'WAIT_ATTACK',
        v_phase_text
    );
    ...
    FROM durak_game
    WHERE game_id = v_game1;
    durak_test.assert_number('После автоматического паса начат второй раунд', 2, v_value);
